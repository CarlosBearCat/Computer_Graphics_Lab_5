...
function render()
{
    ...
    gl.uniform4fv(uLight_position, mult(mult(viewMat,rotateY(lightAngle)),lightObj.light_position));
    ...
    stack.push();
    stack.multiply(rotateY(lightAngle)); // Rotate our light angle about Y
    stack.multiply(translate(6,8,0)); //translate it into this point in space
    stack.multiply(scalem(0.5,0.5,0.5)); //scale our light cube
    gl.uniformMatrix4fv(uModel_view, false, flatten(stack.top()));
    Shapes.drawPrimitive(Shapes.cube); //draw our cube
    stack.pop();

            //draw a floor below our composite object
    stack.multiply(translate(0,-1.5,0));
    stack.multiply(scalem(10,0.25,10));


    gl.uniform4fv(uColor, vec4(0, 1, 1, 1));
    gl.uniformMatrix4fv(uModel_view, false, flatten(stack.top()));
